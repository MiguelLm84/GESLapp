...
public class Inventario_Local_DB extends SQLiteOpenHelper {
    ...
    public int getIdInvent(int position,int id_centro) {
        ...
        Cursor c = db.rawQuery("SELECT id_invent FROM inventario WHERE position = ? AND id_centro = ?",new String[]{String.valueOf(position),String.valueOf(id_centro)});
        if (c.moveToNext()) {
            return c.getInt(0);
    ...
    public ArrayList<Informes> centroselected(int id_centro, String user) {
        ...
        int i = 0;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT fecha_apertura, fecha_cierre, tipo_informe,estado FROM inventario WHERE id_centro = ?", new String[]{String.valueOf(id_centro)});
